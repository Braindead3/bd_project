...
from bd_project.admin_users_table.utils import (set_update_form_fields_values, add_user, update_user,
                                                init_users_forms_and_users)

admin_users_table = Blueprint('admin_users_table', __name__)


@admin_users_table.route('/admin_tables/users/', methods=['GET'])
def users_table():
...
@admin_users_table.route('/admin_tables/users/add', methods=['POST'])
def user_creation():
    users, add_form, delete_form, update_form = init_users_forms_and_users()
    if add_form.add_submit.data and add_form.validate_on_submit():
        add_user(add_form)
        flash('Add successful', 'success')
        return redirect(url_for('admin_users_table.users_table'))
    return render_template('admin_pages/table_users.html', title='users', users=users, add_form=add_form,
                           delete_form=delete_form, update_form=update_form)


@admin_users_table.route('/admin_tables/users/delete', methods=['POST'])
def user_removing():
    users, add_form, delete_form, update_form = init_users_forms_and_users()
    delete_form.select_del_user.choices = [(g.id, g.username) for g in users]
...
@admin_users_table.route('/admin_tables/users/update?<int:user_id>', methods=['GET', 'POST'])
